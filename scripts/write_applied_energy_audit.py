...
VALUES = ROOT / "provenance" / "manuscript_values.csv"
EVIDENCE = ROOT / "provenance" / "case_specific_evidence.csv"
PARETO = ROOT / "results" / "techno_economic" / "practical_pareto_modes.csv"
OUTPUT = ROOT / "docs" / "APPLIED_ENERGY_FIT_NOVELTY_AUDIT.md"


def _display(values: pd.DataFrame, value_id: str) -> str:
    matches = values.loc[values["value_id"].eq(value_id), "display_value"]
    if len(matches) != 1:
        raise ValueError(f"Expected one frozen manuscript value: {value_id}")
    return str(matches.iloc[0])


def main() -> None:
    values = pd.read_csv(VALUES, dtype=str, keep_default_na=False)
    evidence = pd.read_csv(EVIDENCE, dtype=str, keep_default_na=False)
    pareto = pd.read_csv(PARETO, dtype=str, keep_default_na=False)

    status_counts = evidence["status"].value_counts()
    measured = int(status_counts.get("FOUND_MEASURED", 0))
    derivable = int(status_counts.get("FOUND_DERIVABLE", 0))
    bound_only = int(status_counts.get("FOUND_BOUND_ONLY", 0))
    not_found = int(status_counts.get("NOT_FOUND", 0))
    conditional_modes = int(pareto["eligibility"].eq("CONDITIONAL").sum())

    spatial = _display(values, "matched_spatial_value")
    temporal = _display(values, "matched_temporal_value")
    temporal_fraction = _display(values, "matched_temporal_fraction")
    observed_heat = _display(values, "frontier_observed_heat")
    supported_modes = _display(values, "supported_practical_mode_count")

    priority_rows = [
        (
            "Mandatory before submission",
            "No practical mode is supported by case-specific evidence.",
            "High major-revision risk",
            "High",
            "New data needed for closure",
            "Retain NO-GO unless synchronized demand and integrated route, "
            "capture, auxiliary, storage, and environmental evidence appear.",
        ),
        (
            "Mandatory before submission",
            "The live author guide and submission metadata remain incomplete.",
            "Administrative rejection risk",
            "High",
            "Author action required",
            "Recheck the live guide and confirm authorship, affiliation, "
            "funding, conflicts, publication route, repository DOI, "
            "reviewers, and declarations.",
        ),
        (
            "High",
            "Applied Energy fit remains vulnerable without a realizable "
            "integrated case.",
            "High desk-reject risk",
            "High",
            "Requires new evidence or journal repositioning",
            "Position the article as a methods-and-evidence contribution or "
            "target a journal that accepts a transparent negative case result.",
        ),
        (
            "High",
            "Receiving demand is not synchronized with the measured source.",
            "Major-revision risk",
            "High",
            "Requires external data",
            "Acquire or reconstruct an auditable hourly or sub-hourly demand "
            "trace before claiming calibrated useful-heat delivery.",
        ),
        (
            "Medium",
            "Structural scenarios do not exhaust model-form uncertainty.",
            "Moderate revision risk",
            "Medium",
            "Feasible to disclose",
            "Describe replication as prespecified structural sensitivity, not "
            "probabilistic uncertainty quantification.",
        ),
        (
            "Medium",
            "Conditional storage modes use non-case-specific design inputs.",
            "Moderate revision risk",
            "Medium",
            "Feasible to disclose; closure requires design evidence",
            "Keep storage results as sensitivities and do not identify a "
            "preferred design.",
        ),
        (
            "Optional",
            "A synchronized demand trace and project design could calibrate "
            "the applied case.",
            "Not required for a methods-focused fallback",
            "Potentially high",
            "Requires external data",
            "Acquire an auditable load profile and route/equipment design only "
            "if pursuing a stronger Applied Energy claim.",
        ),
    ]
    priority_table = "\n".join("| " + " | ".join(row) + " |" for row in priority_rows)

    text = f"""# Post-freeze Applied Energy fit and novelty audit
...
{supported_modes} practical mode is supported, while all {conditional_modes}
...
The frozen global values are {spatial} burden units/GJ for matched-event
spatial screening and {temporal} burden units/GJ for same-location temporal
screening ({temporal_fraction} of the spatial bound). The measured Frontier
source contributes {observed_heat} MWh-th over valid intervals. These quantities
...
- Strengths: persisted public inputs, immutable snapshots, checksums, generated
  ...
  automated tests.
- The current evidence ledger contains {measured} measured, {derivable}
  derivable, {bound_only} bound-only, and {not_found} not-found case items.
